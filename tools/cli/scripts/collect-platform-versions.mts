...
import type { DaffPackagePlatformVersions } from '../versioning/packages.type';
		...
		Object.entries(packageJson.exports).forEach(([platformExport, overrides]) => {
			...
			if (platform && overrides) {
				Object.keys(overrides).forEach((condition) => {
					...
					if (version) {
						versions[platform] ??= {}
						versions[platform][packageName] = unique([...versions[platform][packageName] ?? [], version])
					}
				})
			}
		})
