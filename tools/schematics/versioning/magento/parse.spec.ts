...
describe('parseMagentoVersion', () => {
  ...
  it('should return null for a missing patch number', () => {
    expect(parseMagentoVersion('2.4.7-p' as MagentoVersionString)).toBeNull();
  });

  it('should return null for a malformed suffix', () => {
    expect(parseMagentoVersion('2.4.7-' as MagentoVersionString)).toBeNull();
  });

  it('should return null for a malformed patch value', () => {
    expect(parseMagentoVersion('2.4.7-ptaco' as MagentoVersionString)).toBeNull();
  });
});
