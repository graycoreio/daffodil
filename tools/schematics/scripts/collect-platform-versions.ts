...
import {
  ...
  DaffVersionString,
  isSupportedPlatform,
} from '../versioning/public_api.js';

const libPath = join(__dirname, '../../../libs');
const outputPath = join(__dirname, '../../../tools/schematics/versioning/packages.json');
      ...
      Object.entries(packageJson.exports).forEach(([platformExport, overrides]) => {
        ...
        if (isSupportedPlatform(platform) && overrides) {
          Object.keys(overrides).forEach((condition) => {
            ...
            if (version) {
              versions[platform] ??= {};
              versions[platform][packageName] = unique([...versions[platform][packageName] ?? [], <DaffVersionString>version]);
            }
          });
        }
      });
