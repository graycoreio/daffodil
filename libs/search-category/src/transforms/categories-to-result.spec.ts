...
describe('@daffodil/search-category | daffTransformCategoriesToSearchResults', () => {
  ...
  it('should create a product list item for each product ID', () => {
    mockCategory.product_ids.forEach(id => {
      expect((<any>result[0].mainEntity).itemListElement).toContain(jasmine.objectContaining({
        '@type': 'ListItem',
