...
export class MockCategory implements DaffCategory {
  ...
  url = `/${enforceUnique(faker.internet.domainWord)}.html`;
  canonicalUrl = faker.internet.url();
  name = faker.commerce.department();
  description = faker.commerce.productDescription();
  meta_title = faker.commerce.department();
  meta_description = faker.commerce.productDescription();
  breadcrumbs = this.breadcrumbFactory.createMany(faker.number.int({ min: 1, max: 5 }));
  children_count = faker.number.int({ min: 1, max: 10 });
  total_products = faker.number.int({ min: 0, max: 9999 });
...
export class DaffCategoryFactory extends DaffModelFactory<DaffCategory, typeof MockCategory>{
  ...
  createTree(depth: number, productIds: DaffProduct['id'][] = [], partial: Partial<DaffCategory> = {}): DaffCategory {
    if (depth > 0) {
      const childrenCount = faker.number.int({ min: 1, max: depth * 2 });

      return this.create({
        children: Array(childrenCount).fill(0).map(() =>
          this.createTree(
            depth - 1,
            randomSubset(productIds, faker.number.int({ min: Math.floor(productIds.length / 2), max: productIds.length })),
          ),
        ),
