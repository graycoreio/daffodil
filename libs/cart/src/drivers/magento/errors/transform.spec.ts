...
fdescribe('Transforming Magento GraphQlErrors into DaffCartErrors', () => {
  ...
  const unhandledGraphQlError = {
    ...
    originalError: null,
    name: "",
    locations: [], path: [1], nodes: [], positions: [] 
  }
  ...
  it('should be able to process graphql errors and return the relevant DaffCart error if a mapping exists', () => {
    ...
    const error = new ApolloError({
      graphQLErrors: [{...unhandledGraphQlError, extensions: { category: "an-unmanaged-error"}}]
    });
    expect(transformError(error)).toEqual(error);
  });
