...
describe('Product | Testing | Factories | DaffCompositeProductFactory', () => {
  ...
  describe('create', () => {
    ...
    it('should return a Composite Product with all required fields defined', () => {
      ...
      expect(result.price.discount.amount).toBeDefined();
      expect(result.price.discount.percent).toBeDefined();
      ...
      expect(result.brand).toBeDefined();
			...
			expect(result.items[0].options[0].price.originalPrice).toBeDefined();
			expect(result.items[0].options[0].price.discountedPrice).toBeDefined();
			expect(result.items[0].options[0].price.discount.amount).toBeDefined();
			expect(result.items[0].options[0].price.discount.percent).toBeDefined();
