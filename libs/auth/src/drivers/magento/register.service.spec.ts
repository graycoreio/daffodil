...
describe('Driver | Magento | Auth | RegisterService', () => {
  ...
  beforeEach(() => {
    ...
    loginServiceSpy.login.withArgs(mockLoginInfo).and.returnValue(mockAuth);
  });
  ...
  describe('register | getting an access token', () => {
    ...
    it('should call DaffLoginDriver#login with the email and password', () => {
      registerService.register(mockRegistration).subscribe((auth) => {
        expect(auth).toEqual(mockAuth);
      });
      ...
      expect(loginServiceSpy.login.calls.mostRecent().args).toEqual([mockLoginInfo])
    });
  });
});
