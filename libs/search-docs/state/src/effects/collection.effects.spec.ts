...
import { DaffDocsStateTestingModule } from '@daffodil/docs/state/testing';
...
import { DaffSearchDocsDriverResponse } from '@daffodil/search-docs/driver';
import { DaffSearchDocsDriverInterface } from '@daffodil/search-docs/driver';
import { DaffSearchDocsTestingDriverModule } from '@daffodil/search-docs/driver/testing';
import { DaffSearchDocsCollectionActionTypes } from '@daffodil/search-docs/state';
import { DaffSearchDocsStateTestingModule } from '@daffodil/search-docs/state/testing';

import { DaffSearchDocsCollectionEffects } from './collection.effects';
...
describe('@daffodil/docs/state | DaffDocsCollectionEffects', () => {
  let actions$: Observable<any>;
  let effects: DaffSearchDocsCollectionEffects;
  let collectionFacade: MockDaffCollectionFacade;
  let facade: MockDaffSearchFacade;
  let driverSpy: jasmine.Spy<DaffSearchDocsDriverInterface['search']>;

  let docsCollectionMetadataFactory: DaffCollectionMetadataFactory;
  ...
  const testDriverSuccess = (cb: () => Action) => {
    describe('throttling the request', () => {
      it('should call immediately, but throttle subsequent events within a specified timeframe, firing off the last event after throttle', () => {
        ...
        const stubCollectionMetadata = docsCollectionMetadataFactory.create({
          filters: daffFilterArrayToDict(filterFactory.createMany(3)),
        });
        const response: DaffSearchDocsDriverResponse = {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [
        DaffSearchStateTestingModule,
        DaffDocsStateTestingModule,
        DaffSearchDocsStateTestingModule,
        DaffSearchTestingDriverModule.forRoot(),
        DaffSearchDocsTestingDriverModule.forRoot(),
      ],
      providers: [
    ...
    effects = TestBed.inject(DaffSearchDocsCollectionEffects);

    driverSpy = spyOn(TestBed.inject(DaffSearchDriver), 'search');

    docsCollectionMetadataFactory = TestBed.inject(DaffCollectionMetadataFactory);
  ...
  describe('when replaceFilters is triggered', () => {
    ...
    beforeEach(() => {
      ...
      action = {
        type: DaffSearchDocsCollectionActionTypes.SearchDocsReplaceFiltersAction,
  ...
  describe('when clearFilters is triggered', () => {
    ...
    beforeEach(() => {
      action = {
        type: DaffSearchDocsCollectionActionTypes.SearchDocsClearFiltersAction,
      };
    });
  ...
  describe('when removeFilters is triggered', () => {
    ...
    beforeEach(() => {
      filterRequest = filterRequestFactory.create();
      action = {
        type: DaffSearchDocsCollectionActionTypes.SearchDocsRemoveFiltersAction,
        filters: [filterRequest],
      };;
    });
