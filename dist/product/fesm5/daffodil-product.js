...
import { __assign, __spread, __decorate, __metadata, __makeTemplateObject } from 'tslib';
...
function getMinimumRequiredCompositeItemPrice(item, qty) {
    return item.required ? daffMultiply(Math.min.apply(Math, __spread(item.options.map((/**
     * @param {?} option
     * @return {?}
     */
...
function getMaximumRequiredCompositeItemPrice(item, qty) {
    return item.required ? daffMultiply(Math.max.apply(Math, __spread(item.options.map((/**
     * @param {?} option
     * @return {?}
     */
    function (option) { return option.price; })))), qty) : 0;
...
function getMinimumRequiredCompositeItemDiscountedPrice(item, qty) {
    return item.required ? daffMultiply(Math.min.apply(Math, __spread(item.options.map(getDiscountedPrice))), qty) : 0;
}
/**
...
function getMaximumRequiredCompositeItemDiscountedPrice(item, qty) {
    return item.required ? daffMultiply(Math.max.apply(Math, __spread(item.options.map(getDiscountedPrice))), qty) : 0;
}
/**
    ...
    return {
        ...
        function (acc, item) { return daffAdd(acc, appliedOptionHasId(appliedOptions[item.id]) ?
            ...
            appliedOptionHasQty(appliedOptions[item.id]) ?
                daffMultiply(Math.max.apply(Math, __spread(item.options.map(getDiscountedPrice))), appliedOptions[item.id].quantity) : Math.max.apply(Math, __spread(item.options.map(getDiscountedPrice)))); }), getDiscountedPrice(product)),
        discount: { amount: null, percent: null },
        ...
        function (acc, item) { return daffAdd(acc, appliedOptionHasId(appliedOptions[item.id]) ?
            daffMultiply(appliedOptions[item.id].price, appliedOptions[item.id].quantity) :
            appliedOptionHasQty(appliedOptions[item.id]) ?
                daffMultiply(Math.max.apply(Math, __spread(item.options.map((/**
                 * @param {?} option
                 * @return {?}
                 */
 ...
 * @type {?}
 */
var DaffProductDriver = new InjectionToken('DaffProductDriver');
